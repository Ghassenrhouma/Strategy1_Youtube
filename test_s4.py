"""
test_s4.py — Strategy 4 test launcher (no watch time, no delays).
...
def main() -> None:
    ...
    print(f"[LAUNCHER] {ts}  Strategy 4 TEST — no watch, no delays", flush=True)
    ...
    ts = datetime.now().strftime("%H:%M:%S")
    print(f"\n[LAUNCHER] {ts}  Strategy 4 test complete.", flush=True)
