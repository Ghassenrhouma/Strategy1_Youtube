"""
test_s1.py — Strategy 1 test launcher (no watch time, no delays).
...
def main() -> None:
    ...
    print(f"[LAUNCHER] {ts}  Strategy 1 TEST — no watch, no delays", flush=True)
    ...
    ts = datetime.now().strftime("%H:%M:%S")
    print(f"\n[LAUNCHER] {ts}  Strategy 1 test complete.", flush=True)
