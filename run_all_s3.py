"""
run_all_s3.py — Strategy 3 launcher (2-account debate thread).

Account 1 opens with a position on a polarizing trade topic.
Account 2 takes the opposing side. They trade 3-5 replies back and forth.
DocShipper is cited exactly once per thread as a data source.

Usage:
    python run_all_s3.py
"""
...
BOTS = [
    ("S3-A1", "main_s3_account1.py"),
    ("S3-A2", "main_s3_account2.py"),
]
...
def main() -> None:
    ...
    print(f"[LAUNCHER] {ts}  Strategy 3 — opposing positions debate (3-5 turns)", flush=True)
    ...
    ts = datetime.now().strftime("%H:%M:%S")
    print(f"\n[LAUNCHER] {ts}  Strategy 3 complete.", flush=True)
